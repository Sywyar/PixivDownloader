(function (global) {
    ...
    var ICON_PATHS = {
        ...
        // 通用图标，供任意导航贡献按 icon token 引用：grid（网格）/ users（用户组）。
    ...
    function collectNamespaces(items) {
        ...
        items.forEach(function (it) { seen[namespaceOf(it.labelI18nKey)] = true; });
    ...
    function resolveLabel(i18n, item) {
        if (i18n && typeof i18n.t === 'function') {
            return i18n.t(item.labelI18nKey, item.id);
        }
        return item.id;
    }
    ...
    function buildItemHtml(item, label, isCurrent, opt) {
        var cls = opt.linkClass + (isCurrent ? (opt.linkClass ? ' ' : '') + opt.activeClass : '');
        // icon 渲染：noIcon → 不渲染；否则按是否有 wrap class 决定外层。
        var iconHtml = '';
        if (!opt.noIcon) {
        ...
        return '<a class="' + escapeAttr(cls) + '"' + attrs + '>' + iconHtml + labelHtml + '</a>';
    }

    function renderSlot(slot, items, i18n) {
        var placement = slot.getAttribute('data-nav-slot');
        if (!placement) { slot.innerHTML = ''; return; }
        var explicitCurrent = slot.getAttribute('data-nav-current'); // null 表示未声明 → 按 pathname 推断
        var opt = {
        ...
        var html = items.filter(function (it) {
            ...
            var isCurrent = explicitCurrent != null
                ? (it.id === explicitCurrent)
                : (baseHref(it.href) === pathname);
            return buildItemHtml(it, resolveLabel(i18n, it), isCurrent, opt);
        }).join('');
        slot.innerHTML = html;
    }
    ...
    async function renderFromState() {
        ...
        if (state.items == null) {
            // 失败降级：清空全部 slot，不残留硬编码坏入口。
            slots.forEach(function (s) { s.innerHTML = ''; });
        } else {
            var i18n = await buildI18n(state.items);
            slots.forEach(function (slot) { renderSlot(slot, state.items, i18n); });
        }
        // 每次渲染完成（含失败降级）后派发稳定事件：动态 slot 链接此时已就位，宿主页可据此同步 href / 登记跨页交接，
        // 不必猜测异步渲染时机。失败降级时 slot 已清空，监听方对零链接做无操作处理即可。
        dispatchRendered();
    }
    ...
    global.PixivNav = {
        mount: mount,
        refresh: refresh,
        // 首次渲染完成（成功或失败）后 resolve；供宿主在导航数据到位、slot 链接渲染后再做处理。
