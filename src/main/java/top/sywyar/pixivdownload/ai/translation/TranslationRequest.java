 ...
 * <b>业务变量</b>（目标语言、待翻译文本、可选随段同译的标题、名词映射表）与 <b>输出规范</b>（提示词 +
 * 期望的 JSON 结构）封装在一起，与底层 OpenAI 兼容协议解耦。
 *
...
public record TranslationRequest(String targetLanguage, String sourceText,
    ...
    public TranslationRequest(String targetLanguage, String sourceText) {
        this(targetLanguage, sourceText, null, List.of());
    }

    /** 便捷构造：无标题、含名词映射表。 */
    public TranslationRequest(String targetLanguage, String sourceText, List<GlossaryTerm> glossary) {
        this(targetLanguage, sourceText, null, glossary);
    }

    /**
     * 系统提示词（英文，固定）。明确翻译规则、标记保留规则、名词映射规则、标题随段同译规则与 JSON 输出规范。
     * 业务变量（目标语言、映射表、待译文本、可选标题）由 {@link #toMessages()} 在用户消息中给出，这里只定义不变的规则。
     */
    private static final String SYSTEM_PROMPT = """
            ...
            and (when also provided) translate the work title in the SAME request.
            ...
            1. First decide whether the requested target language is a real, existing human (natural) language.
               ...
               respond with status "invalid_language", leave "text" as an empty string and "title" as null.
               Do not translate.
            ...
            6. A glossary of preferred term translations may be provided in the user message as a JSON array
               ...
               that "source" term exactly as its "target" everywhere it appears (in body AND title). Ignore
               items whose "lang" differs from the target language.
            ...
            8. If the user message includes a "Title to translate" line, ALSO translate it into the same
               ...
               set "title" to null.
            9. Output STRICT JSON ONLY (no markdown, no code fences, no extra text) with EXACTLY these fields:
