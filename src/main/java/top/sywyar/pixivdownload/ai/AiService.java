 ...
 *       {@link AiException}（调用方决定如何处理）</li>
 *   <li>{@link #chatTest} —— GUI 连通性测试路径，使用调用方传入的临时 {@link AiClientSettings}（不读取
 *       {@link AiConfig}，也不检查总开关），失败抛 {@link AiException}</li>
 * </ul>
...
public class AiService {
    ...
    private AiChatResult deliver(String callType, AiClientSettings settings,
        ...
        if (settings.model() == null || settings.model().isBlank()) {
            throw new AiException(localized("ai.error.model-missing"));
        }
        if (chatMessages == null || chatMessages.isEmpty()) {
            throw new AiException(localized("ai.error.messages-missing"));
        }
    ...
    private AiChatResult parseResult(byte[] body) throws AiException {
        if (body == null || body.length == 0) {
            throw new AiException(localized("ai.error.empty-response"));
        }
        ChatResponse response;
        try {
            response = MAPPER.readValue(body, ChatResponse.class);
        } catch (Exception e) {
            throw new AiException(localized("ai.error.empty-response"), e);
        }
    ...
    private byte[] serialize(ChatRequest request) throws AiException {
        try {
    ...
    /** 调用失败时透传给调用方 / GUI 的异常；message 已脱敏、截断，绝不含 API Key。 */
