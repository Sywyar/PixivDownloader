 ...
 * {@link PushService 多通道下发}」这套构建链。消息 id / 级别取自场景的
 * {@link NotificationScenario#id()} / {@link NotificationScenario#level()}。
 *
 * <p>{@link #deliver} best-effort：渲染异常兜底（{@code PushService.push} 本身也不抛、各通道失败已隔离）；
...
public class PushNotificationSink implements NotificationSink {
    ...
    /** i18n 缺失时 {@code AppMessages} 回退为 key 本身：空 / 等于 key 即视为缺失。 */
