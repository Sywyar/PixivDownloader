...
public class ArtworkFileLocator {
     ...
     * {@code _thumb.jpg} 缩略图、动图的 {@code _thumb.jpg}。这些文件经共享 {@link StagedFileDeletion}
     * <b>原子删除</b>（先暂存再删，任一删除失败回滚到删除前状态）——失败时作品文件原样保留、不会半损坏 / 裂图。
     ...
     * 按文件名前缀（{@code stems}）枚举式匹配，即使目录是共享分类目录也只会匹配到本作品命名空间内的文件。
     */
