 ...
 * <p>
 * 端点按官方 SDK 自适应：{@code sctp} 前缀的 SendKey 走 {@code https://{key}.push.ft07.com/send}（Server 酱³），
 * 否则走 {@code https://sctapi.ftqq.com/{key}.send}（Turbo）。只读取 {@link ServerChanConfig}，与其它通道解耦；
...
public class ServerChanPushChannel implements PushChannel {
    ...
    private PushResult deliver(ServerChanSettings settings, PushMessage message) {
        ...
        String form = "title=" + enc(message.title()) + "&desp=" + enc(message.content());
