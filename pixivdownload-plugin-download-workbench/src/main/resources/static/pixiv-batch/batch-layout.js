...
function applyBatchLayout(layout, options) {
    ...
    const previousLayoutToken = previousLayout == null || layouts.indexOf(previousLayout) < 0
        ? null
        : previousLayout;
    if (previousLayoutToken !== normalized) {
        dispatchBatchLayoutChanged(normalized, previousLayoutToken);
    }
    return normalized;
}

function dispatchBatchLayoutChanged(layout, previousLayout) {
    let event = null;
    try {
        event = new CustomEvent('pixiv:batch-layout-changed', {detail: {layout, previousLayout}});
    } catch (_) {
        event = {type: 'pixiv:batch-layout-changed', detail: {layout, previousLayout}};
    }
    try {
