...
'use strict';
/**
 * 可信 base 解析（CI 共享实现；Gate Epoch 4 单一标准）。
 *
 ...
 *
 * Epoch 4 规则（与 .github/workflows/quality-gate.yml 的 bootstrap 保持一致）：
 * 0. 解析 root tag refs/tags/i18n-gate-epoch-4-root^{commit}：
 ...
 *      （"Gate Epoch 4 trust root has not been installed."）；
 ...
 *    - 其它（不包含 Epoch 4 root 的 candidate）→ fail closed，不尝试任何旧 Epoch 路径。
...
function assertSupportedTrustedVerifier(repoRoot, base, minimum) {
    ...
    if (policyText === null) {
        fail('trusted base ' + base + ' has no gate-policy.json; it does not satisfy the'
            + ' current Gate Epoch 4 verifier baseline; fail closed');
        return;
    }
    let policy;
    try {
    ...
    if (policy.gateEpoch !== 4) {
        fail('trusted base ' + base + ' gateEpoch ' + policy.gateEpoch
            + ' != 4; it does not satisfy the current verifier baseline; fail closed');
        return;
    }
...
function main() {
    ...
    if (args.version) {
        console.log('resolve-trusted-base 5');
        return;
    }
    ...
    if (!root) {
        ...
        if (admission) {
            mode = 'ROOT_ADMISSION';
                ...
                fail('Gate Epoch 4 trust root has not been installed'
                + ' (refs/tags/i18n-gate-epoch-4-root missing); only an explicit'
        ...
        fail('candidate does not descend from the Gate Epoch 4 trust root (' + root
    ...
    if (mode === 'ROOT_ADMISSION') {
        base = root || candidate;
        console.error('ROOT ADMISSION MODE: candidate is the Gate Epoch 4 trust root candidate'
        ...
        base = resolved;
        // trusted base 必须包含 Epoch 4 root，不允许回退到旧 Epoch 历史。
        if (!isAncestor(repoRoot, root, base)) {
            fail('trusted base ' + base + ' does not descend from the Gate Epoch 4 trust root '
                + root + '; fail closed');
            return;
        }
