...
| 社区候选归档与复用 | `api.github.com` 的同仓库 PR、run/attempt、job/log、artifact、Release/asset API；`uploads.github.com` 及 GitHub 返回的资产下载地址 | 成功版本构建唤醒受保护归档 job。它用本 job 的内置 token 重新核验原生身份、执行来源及原始字节，按发布者数字账号与类型、发布者 ID、插件 ID 和版本复用待审核 Draft Release，更新包、源码与证据，不执行投稿代码。替换先删除旧证明，再更新不同字节的资产、候选清单和新证明；关闭请求后重新投稿继续复用，另一开放或已合并请求不能被覆盖。旧 PR 格式草稿可就地迁移，其余历史草稿不自动删除。归档与完成审核、正式发布共用队列。自审、Gate 和预检用仅执行主线代码的 Contents write 作业读取 Draft；构建作业仍只读，无法读取草稿时执行隔离重建。人工审核等待和正式发布使用已归档原包，不重新构建。API / 下载使用 GitHub CLI，沿用其认证、代理及平台重定向；Actions artifact ZIP 请求接受 GitHub JSON 媒体类型以获取重定向，Release asset 请求接受原始二进制；单调用六十秒，归档 job 十五分钟。Draft 没有脚本设定的过期清理，不进入公开目录。停用归档会阻断需要新归档的版本审核 |
