...
| 工具/流程 | 目标或来源 | 用途 |
| --- | --- | --- |
| Release / Nightly 发行物 E2E | GitHub Actions Artifact 服务，上传地址和重定向由 runner 提供 | 验收成功或失败后，自动上传隔离测试实例的结果、日志、窗口截图和插件状态，保留七天。上传使用 Actions 为当前 job 提供的运行时凭据，不上传发布私钥；目标和代理遵循 runner 与官方 upload-artifact Action 的配置。维护者可修改或停用共享发行验收 Action 中的证据上传步骤 |
| Git 和发布脚本 | 当前 `origin`、`https://api.github.com`（或 `GITHUB_API_URL`）、GitHub Release | `fetch`、远端引用检查、质量门禁审计、插件和应用发布 |
| GitHub Actions | GitHub Actions、Artifact、Release 服务以及 workflow 引用的 `actions/*`、`softprops/action-gh-release` | CI、构建、上传产物和发布。完整 Quality Gate 的 Java 测试 / SDK 合同与 ProGuard 产物验证在独立 job 中并行运行，分别 checkout 同一源码并使用 Maven 依赖缓存；冷缓存可能重复下载依赖。两者仅有源码读取权限，不使用发布凭据，不在 job 之间传递构建产物；下载目标与代理边界仍由下列构建工具配置决定 |
