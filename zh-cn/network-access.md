...
| 社区投稿 PowerShell 入口 | `https://raw.githubusercontent.com/Sywyar/PixivDownloader-community-plugins/<完整commit>/` 下固定清单及工具文件 | 用户手动运行 `tools/submit.ps1`，先验证 SDK 工程标识，再读取固定提交和 SHA-256 对应的工具。已有本地文件或缓存通过逐文件大小与摘要核对后直接复用；缺失时匿名下载，遵循平台默认代理设置，不携带 Cookie 或默认凭据，不接受重定向，每个请求最多六十秒。缓存保存在本地应用数据目录；不运行入口即可避免访问，更换工具须同时更新来源提交与清单摘要 |
