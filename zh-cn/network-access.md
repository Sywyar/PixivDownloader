...
| 社区候选归档与复用 | `api.github.com` 的同仓库 PR、run/attempt、job/log、artifact、Release/asset API；`uploads.github.com` 及 GitHub 返回的资产下载地址 | 成功版本构建唤醒受保护归档 job。它用本 job 的内置 token 重新核验原生身份、执行来源及原始字节，再创建或补齐待审核 Draft Release；只上传包、源码与证据，不执行投稿代码。只读构建和 Gate 可重新下载已认证 Draft 的固定字节，人工等待与相同构建输入不要求重建。API / 下载使用 GitHub CLI，沿用其认证、代理及平台重定向；Actions artifact ZIP 请求接受 GitHub JSON 媒体类型以获取重定向，Release asset 请求接受原始二进制；单调用六十秒，归档 job 十五分钟。Draft 没有脚本设定的过期清理，不进入公开目录。停用归档会阻断需要新归档的版本审核 |
