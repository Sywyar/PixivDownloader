...
| 社区审核结果执行与续签 | 同一社区仓库的 PR、Review、协作者、Environment、Actions 批准历史、Git tree/blob/commit/ref、Release/asset API；`/user/<accountId>`、`/organizations/<accountId>`；GitHub 上传与资产 CDN；上述 OIDC、attestation 和公共信任根服务 | 维护者在 `master` 手动运行 `Apply reviewed community operation`，预检已合并请求后批准 `release` Environment。只读签名步骤取得原生事实及冻结原包，在本地调用固定 SDK；独立写入步骤用社区所有者令牌保存结果 Draft、提升正式 Release 并创建待审核结果 PR。请求包含审核、恢复及组织代表证据、公钥、签名和包字节，私钥仅经 stdin 进入本地签名进程。正式包还经匿名 HTTPS 下载复核，沿用上面的 Node 代理与下载边界。API、上传和认证资产下载沿用 GitHub CLI 的代理、重定向及单次六十秒预算；预检十五分钟，执行三十分钟。结果原文及证明各最多 32 MiB，Draft 无自动到期删除。`action=refresh` 复用相同批准与结果 PR 路径续签三十天撤销清单，不更新原包。停用 workflow 会阻断新的执行和续签 |
| 社区 Release 状态收尾 | 同一社区仓库的 Git、Release/asset、PR、标签与评论 API，以及上述归档证明验证服务 | `master` push 或已执行请求的恢复触发；只读取主线已合并结果及资产身份，使用内置 job token 更新 Release 标题、说明和请求状态，不覆盖包、签名或历史归属。执行失败时另以同一原生请求回写失败提示，保留原准入检查。每次 job 三十分钟，API 沿用 GitHub CLI 预算与代理；回读错误保持失败，不能把部分远端写入视为原子完成。维护者可停用 workflow，此时 Release 状态可能尚未与签名目录同步 |
