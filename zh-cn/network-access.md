...
| 主仓库 `Gate Checks` workflow | `https://api.github.com/repos/Sywyar/PixivDownloader/` 下的 Actions run、job、日志、PR、commit 与 check-runs API，GitHub App 安装及 token API，当前 `origin`，以及日志 API 返回的下载重定向 | 自动响应订阅的 CI 事件；PR-only 启用时也验证 `master` 合并结果，并接收维护者手动运行的同一提交全量 Quality Gate 结果。使用只读 `GITHUB_TOKEN` 取得执行证据，以 Git 获取对应提交对象；校验通过后，官方 `actions/create-github-app-token` 用 `release` Environment 中的 App 私钥在 runner 内签发 JWT，再向 GitHub 换取仅有 `checks:write` 的短期安装 token，在被测 PR 合并提交上更新六项检查。私钥不发送给 GitHub，token 在 job 结束时撤销。发送的数据包括仓库、PR / run / job 编号、提交 SHA 和检查结论。请求使用 runner 的 Git / GitHub CLI / 代理配置；日志下载目标由 GitHub 返回。停用 workflow 或 App 会阻塞对应 required checks，不是跳过质量门禁的方式 |
