...
所有权转移由接收方创建一个申请 PR。原所有者运行向导时读取开放 PR、固定 head 的请求文件和当前绑定，确认后用自己的 GitHub CLI 凭据向该 PR 的 `/reviews` API 提交同意或拒绝。正文包含决定类型、请求摘要，以及用户选择提供的活动密钥签名；私钥和密码不上传，也不需要访问接收方源码仓库。写入失败先回读 Review，避免重复提交。原生 Review 事件可唤醒受保护自动流程；不同个人账号的双方签名均有效时，在原 PR 追加结果并按保护规则合并。Gate 和最终合并重新读取原生身份、Review、当前密钥及紧急状态；缺少证明、组织和恢复请求仍走人工审核。通知作业通过数字账号查询当前 login，在唯一的请求信息评论中提醒原个人所有者；明确拒绝时，重新核对当前绑定、请求与决定，再关闭原 PR。通知使用既有 `pull-requests: write` job token，自动追加和合并使用既有受保护维护者凭据。所有请求沿用 GitHub CLI 的代理、认证及六十秒和 32 MiB 预算；不运行向导或停用相关 workflow 即可停止相应请求。
