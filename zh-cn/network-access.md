...
| SDK 构建的固定官方输入 | `https://api.github.com/repos/Sywyar/PixivDownloader/releases/assets/<asset-id>` 及 GitHub 发行 CDN | QG 和新 SDK 发布构建通过 curl 下载源码中锁定的资产 ID，验证长度与 SHA-256，提取官方插件及原 provenance，交由现有完整分发组装器复验。请求无发布凭据，使用 curl / 构建机代理，最多 5 次 HTTPS 重定向；本地可传入完全匹配的 `-Archive` 避免联网。更换配套输入须修改锁定元数据并重新验证，缺失时失败，不按移动的 nightly 标签重选。已发布 SDK 的开发者取得上行所述 SDK 仓库固定附件 |
| SDK 冻结附件与发布恢复 | 上述 SDK 仓库 Release、Tag 与资产 API | Central 前持久保存已签名 Draft 附件并回读；全部公开坐标可读后，验证冻结附件与完整宿主运行，再公开同一 Release。恢复仅复用原始宿主、工具和 SDK ZIP，缺失或冲突失败，不覆盖公开资产。发布后消费者用空缓存实际下载运行附件。跨仓库 token 只用于发布 workflow 的 GitHub 请求；Pages 匿名下载 SDK / 历史 Javadoc ZIP，校验运行附件元数据，不下载历代宿主 |
