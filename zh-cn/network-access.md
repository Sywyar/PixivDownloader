...
| 已签名版本状态自动执行 | 社区仓库的 GitHub PR、用户、环境规则、Git 对象、Release/asset、check-runs、Actions dispatch API，以及官方 Action 的证明与工具下载服务 | `Apply signed version status` 由静态检查完成唤醒，只读取请求数据，以受保护 `master` 工具核验个人管理者签名。专用 `community-status` Environment 的社区根仅传入签名步骤；已有维护者分支令牌用于追加 fork 提交和以所有者身份合并，job token 用于其它 API 与显式唤醒 Gate / 发布收尾。等待检查最多十分钟，合并绑定精确 head 并保留保护规则；不重新构建插件、不自动删除分支。请求包含操作、PR、SHA、签名结果及状态，不发送私钥。代理和下载目标由 GitHub CLI、官方 Action 与 runner 设置决定。禁用该 workflow 可停止自动执行，人工流程仍可使用 |
