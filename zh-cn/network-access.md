...
| 工具/流程 | 目标或来源 | 用途 |
| --- | --- | --- |
| 投稿教程的 `irm ... \| iex` | `https://raw.githubusercontent.com/Sywyar/PixivDownloader-community-plugins/<完整commit>/tools/submit.ps1` | 用户在 SDK 工程目录手动执行教程命令，PowerShell 原生 `Invoke-RestMethod` 下载固定提交的 UTF-8 入口，再交 `Invoke-Expression` 执行。该次下载遵循 PowerShell 的网络、代理、重定向和超时设置，教程不传入 token、私钥或请求正文；入口执行后才检查工程标识并进入下列固定工具下载路径。不运行命令即可避免访问 |
| 社区投稿 PowerShell 入口 | `https://raw.githubusercontent.com/Sywyar/PixivDownloader-community-plugins/<完整commit>/` 下固定清单及工具文件 | 用户手动运行 `tools/submit.ps1`，先验证 SDK 工程标识，再读取固定提交和 SHA-256 对应的工具。已有本地文件或缓存通过逐文件大小与摘要核对后直接复用；缺失时匿名下载，不携带 Cookie、默认凭据或代理，不接受重定向，每个请求最多六十秒。缓存保存在本地应用数据目录；不运行入口即可避免访问，更换工具须同时更新来源提交与清单摘要 |
| 本地社区投稿向导 | `api.github.com` 的用户、组织成员关系、仓库、Git tree/blob、commit、Release/asset、fork 与 PR API；`https://github.com/` 上固定社区仓库和当前账号 fork 的 Git HTTPS 地址；公开源码归档与用户选择的包地址 | 用户主动启动，GitHub CLI 使用当前登录读取原生身份和受保护数据；包与源码下载沿用静态检查的匿名公网下载边界。完整预览确认并复核当前事实后才创建所需 fork、推送投稿文件并创建 Ready PR，不自动批准或合并。API 与 Git 分别使用本机 CLI 认证、代理及 Git 配置；发送投稿元数据、公钥、签名和选定静态图片或恢复证据，私钥留在本机。取消最终预览可阻止 GitHub 写入，不运行向导可避免全部请求 |
| 本地投稿模型查询 | Maven、Gradle、sbt 的工程配置、Wrapper 与本机配置指定的仓库、镜像和工具分发地址，包括 Maven Central 和工具自身的下载服务 | 只有开发者确认信任当前工程后才执行模型查询；可能下载构建插件、依赖和 Wrapper 工具。查询使用本地账号权限，连接、缓存、代理及凭据由构建工具和工程配置决定；向导过滤常见 token、secret、私钥与 Java 注入环境变量，这不限制工程访问本地文件的能力。拒绝信任确认可避免执行；无密钥 PR 静态检查不调用这些构建工具 |
| 社区投稿静态检查 | `api.github.com` 的社区仓库 PR、Git tree/blob、源码仓库 commit 与数字账号 API；投稿中的公开 HTTPS 安装包地址；固定 commit 的 GitHub 源码 ZIP 及其重定向；Actions artifact 服务 | PR 事件运行受保护默认分支的固定检查器，使用只读 token 获取身份和原始数据，不执行投稿代码或构建。包和源码下载不携带 GitHub 凭据或 Cookie，不使用代理；每跳验证并固定公网 IP、保持原主机 TLS 验证，最多五次重定向、总计六十秒，并检查实际大小和 SHA-256。API 连接使用 GitHub CLI 配置。静态通过只允许继续进入版本构建；版本构建完成后的交接 artifact 保留七天，上传使用平台当前 job 的运行时凭据。维护者可停用此 workflow，静态结果本身不授予准入或发布权限 |
| 社区构建工具与容器准备 | Maven / sbt 的固定版本位于上述 Maven Central；Gradle 位于 `services.gradle.org/distributions/` 及其 GitHub/CDN 重定向；Node.js 位于 `nodejs.org/dist/`；Docker Hub 的 `eclipse-temurin`、`ubuntu/squid` 镜像及其认证/CDN 服务 | 版本投稿或维护者请求重算时，在执行投稿代码前匿名下载工具并核对固定 SHA-256；单个工具最多 192 MiB、总下载六十秒、最多五次重定向，每跳固定公网 IP。Docker 原生客户端按完整镜像摘要拉取，registry 认证、代理、缓存与重定向遵循 runner 配置，不能将其下载目标当作投稿容器白名单。发送公共工具路径、版本、镜像摘要及平台信息；不发送投稿源码或社区私钥。维护者通过受保护 SDK 锁和构建策略更新工具，停用构建 workflow 可避免访问 |
| 社区构建依赖预取 | 仅 `repo.maven.apache.org`、`repo1.maven.org`、`plugins.gradle.org`、`plugins-artifacts.gradle.org` 的 HTTPS 443 | Maven / Gradle / sbt 在无凭据容器内执行投稿构建配置，通过本次独立 Squid 代理请求批准主机上的制品路径。路径由实际工程及构建工具决定；代理拒绝非公网地址、非 HTTPS CONNECT 与其它主机，容器没有外部 DNS、宿主网桥出口或共享可写缓存。正式重建使用新建的断网容器，不产生外部请求。默认随版本构建执行，单次预取与重建各限时三十分钟；更改批准源须修改受保护构建策略 |
| 社区候选归档与复用 | `api.github.com` 的同仓库 PR、run/attempt、job/log、artifact、Release/asset API；`uploads.github.com` 及 GitHub 返回的资产下载地址 | 成功版本构建唤醒受保护归档 job。它用本 job 的内置 token 重新核验原生身份、执行来源及原始字节，再创建或补齐待审核 Draft Release；只上传包、源码与证据，不执行投稿代码。只读构建和 Gate 可重新下载已认证 Draft 的固定字节，人工等待与相同构建输入不要求重建。API / 下载使用 GitHub CLI，沿用其认证、代理及平台重定向；单调用六十秒，归档 job 十五分钟。Draft 没有脚本设定的过期清理，不进入公开目录。停用归档会阻断需要新归档的版本审核 |
