 ...
 * {@link #verifyRenderable} 通过实际 {@link MailTemplateRegistry#render 渲染}（空占位符）校验场景插件
 * 已为中英文贡献可用的标题与 HTML 正文。
 */
@Component
...
public class MailNotificationSink implements NotificationSink {
    ...
    public void verifyRenderable(NotificationScenario scenario) {
        for (Locale locale : VERIFY_LOCALES) {
            RenderedMail mail;
            try {
                // render 内部：场景插件未贡献 → IllegalArgumentException。
            ...
            if (subject == null || subject.isBlank() || mail.htmlBody() == null || mail.htmlBody().isBlank()) {
                throw new IllegalStateException(
                        "mail template missing for scenario " + scenario.id() + " @ " + locale);
            }
        }
    }
}
