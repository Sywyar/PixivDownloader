...
/**
 * 保存浏览器已取得的小说元数据，并用短期一次性票据把导入与下载请求关联起来。
 */
public final class NovelBrowserFetchTicketStore {
    ...
    public synchronized String issueFetchTicket(
        ...
        fetchTickets.put(token, new Ticket(
                novelId, new ImportedNovel(metadata, rawMetaJson), expiresAt()));
        return token;
    }

    public synchronized Optional<ImportedNovel> consumeFetchTicket(String token, long novelId) {
        pruneExpired();
    ...
    public record ImportedNovel(PixivNovelMetadata metadata, String rawMetaJson) {
    }

    private record Ticket(long novelId, ImportedNovel importedNovel, Instant expiresAt) {
    }
}
