...
- Scheduled tasks (admin): automatically discover and download new artworks in the background on a fixed interval or cron schedule, with three source types (artist new works / saved search / series — both illustrations and novels); save one with "Save as scheduled task" from User/Search/Series mode and it inherits all current download and filter settings, applying them per-work in the background, with per-task authorization to use the admin's saved cookie
