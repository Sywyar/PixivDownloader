...
public class ConfigPanel extends JPanel implements ConfigSectionContext {
    ...
    private final Map<String, String> interfacePreferenceDraft;
    ...
    private InterfacePreferencesPanel interfacePreferencesPanel;
    ...
    public ConfigPanel(Path configPath, int serverPort, Function<String, String> webUrlProvider,
                       ...
                       Runnable onLocaleChanged,
                       BooleanSupplier languageChangeBlocked,
                       Map<String, String> interfacePreferenceDraft) {
        this(configPath, serverPort, webUrlProvider, fieldSnapshot, onLocaleChanged, languageChangeBlocked,
                null, SwingBackendLifecycle::restartAsync, null, null, interfacePreferenceDraft);
    }
    ...
    ConfigPanel(Path configPath, int serverPort, Function<String, String> webUrlProvider,
        ...
        this(configPath, serverPort, webUrlProvider, fieldSnapshot, onLocaleChanged, languageChangeBlocked,
                restartConfirmation, backendRestarter, null, null, new LinkedHashMap<>());
    }
    ...
    ConfigPanel(Path configPath, int serverPort, Function<String, String> webUrlProvider,
        ...
        this(configPath, serverPort, webUrlProvider, fieldSnapshot, onLocaleChanged, languageChangeBlocked,
                restartConfirmation, backendRestarter, processRestartConfirmation, processRestarter,
                new LinkedHashMap<>());
    }

    private ConfigPanel(Path configPath, int serverPort, Function<String, String> webUrlProvider,
                        ConfigFieldSnapshot fieldSnapshot,
                        Runnable onLocaleChanged,
                        BooleanSupplier languageChangeBlocked,
                        BooleanSupplier restartConfirmation,
        ...
        this.processRestarter = processRestarter == null
                ...
                : processRestarter;
        this.interfacePreferenceDraft = Objects.requireNonNull(
                interfacePreferenceDraft, "interfacePreferenceDraft");
    ...
    private void buildUi() {
        ...
        interfacePreferencesPanel = new InterfacePreferencesPanel(
                configPath, onLocaleChanged, languageChangeBlocked, this::setExpandAllConfigMenus,
                interfacePreferenceDraft);
        expandAllConfigMenus = interfacePreferencesPanel.isExpandAllSelected();
        menuRoots.add(ConfigMenuTree.leaf(
                "interface", message("gui.config.category.interface"), interfacePreferencesPanel));
        ...
        try {
            ...
            for (ConfigFieldSpec spec : allFields) {
                ...
                if (isPluginCredential(spec)) {
                    rf.setCredentialStored(values.containsKey(spec.key())
                            && !normalizeValue(values.get(spec.key())).isBlank());
                }
            }
    ...
    private void saveConfig() {
        ...
        Map<String, String> interfaceBefore;
        try {
            before = readStoredValues(allFields);
            interfaceBefore = editor.readAll(InterfacePreferencesPanel.CONFIG_KEYS);
        } catch (IOException e) {
            log.warn(logMessage("gui.config.log.read-failed", e.getMessage()));
            GuiErrorDialog.show(this,
                    message("gui.dialog.error.title"),
        ...
        boolean hasHotReloadChanges = !hotReloadKeys.isEmpty()
                || interfaceChangedKeys.stream().anyMatch(key ->
                        !InterfacePreferencesPanel.GUI_PROVIDER_CONFIG_KEY.equals(key));
        ...
        boolean hasProcessRestartChanges = hasChangedField(changedKeys, GuiConfigEffect.PROCESS_RESTART)
                || interfaceChangedKeys.contains(InterfacePreferencesPanel.GUI_PROVIDER_CONFIG_KEY);
        ...
        try {
            writeStoredValues(values, interfaceValues);
            refreshCredentialFieldsAfterSave(values);
    ...
    private void writeStoredValues(Map<String, String> values) throws IOException {
        writeStoredValues(values, Map.of());
    }

    private void writeStoredValues(Map<String, String> values,
                                   Map<String, String> interfaceValues) throws IOException {
        StoredValuesSplit split = splitStoredValues(values);
        if (interfaceValues != null && !interfaceValues.isEmpty()) {
            Map<String, String> coreValues = new LinkedHashMap<>(split.coreValues());
            for (Map.Entry<String, String> entry : interfaceValues.entrySet()) {
                String key = SwingHost.host().requireSafeConfigKey(entry.getKey());
                if (!InterfacePreferencesPanel.CONFIG_KEYS.contains(key)) {
                    throw new IOException("Unknown interface preference key: " + key);
                }
                coreValues.put(key, SwingHost.host().requireSafeConfigValue(entry.getValue()));
            }
            split = new StoredValuesSplit(coreValues, split.pluginValues(), split.credentialValues());
        }
        if (split.empty()) {
            return;
        }
        StoredValuesSplit pendingWrites = split;
        SwingHost.host().withCredentialLocks(
                pendingWrites.credentialValues().keySet(),
                () -> writeStoredValuesWithCredentialLocks(pendingWrites));
    }

    private void writeStoredValuesWithCredentialLocks(StoredValuesSplit split) throws IOException {
        ...
        try {
            ...
            for (Map.Entry<String, Map<String, String>> entry : split.credentialValues().entrySet()) {
                ...
                Set<String> clearKeys = new LinkedHashSet<>();
                ...
                // on the next migration instead of a stale properties/YAML value being resurrected.
                if (!clearKeys.isEmpty()) {
                    pluginEditor.removeAll(clearKeys);
                    if (!pluginEditor.readAll(clearKeys).isEmpty()) {
                        throw new IOException("Plugin credential cleanup verification failed for owner: "
                                + entry.getKey());
                    }
                    editor.removeAll(clearKeys);
                    if (!editor.readAll(clearKeys).isEmpty()) {
                ...
                if (!pluginEditor.readAll(setKeys).isEmpty()) {
                    throw new IOException("Plugin credential cleanup verification failed for owner: "
                            + entry.getKey());
                }
                credentialYamlKeys.addAll(setKeys);
            }
    ...
    private void migrateLegacyPluginCredentialsWithOwnerLock(
            String pluginId,
    ...
    private void refreshCredentialFieldsAfterSave(Map<String, String> values) {
        for (ConfigFieldSpec spec : allFields) {
            if (!isPluginCredential(spec)) {
                continue;
            }
            ...
            boolean stored = values.containsKey(spec.key())
                    ? !normalizeValue(values.get(spec.key())).isBlank()
                    : rf.credentialStored();
            rf.setValue().accept("");
            rf.setCredentialStored(stored);
        }
    }
    ...
    private void resetToDefaults() {
        ...
        for (ConfigFieldSpec spec : allFields) {
            FieldRenderer.RenderedField rf = renderedFields.get(spec.key());
            if (rf != null) {
                rf.setValue().accept(isPluginCredential(spec) ? "" : spec.defaultValue());
