...
中文 | [繁體中文](./README_zh-Hant.md) | [日本語](./README_ja.md) | [English](./README_en.md)
...
- `*` 插件管理页：卡片列表展示所有插件的状态/来源/版本/依赖，支持外部插件的生命周期操作（加载/启动/停止/卸下/重载）（未上线）
- `*` 插件市场页：浏览/搜索/筛选受信仓库插件，查看详情并安装（重启生效）；仓库列表可在桌面 GUI 配置页维护（未上线）
