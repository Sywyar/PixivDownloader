...
- 计划任务（管理员）：后台按固定周期或 Cron 表达式自动发现并下载新作品，支持画师新作/保存的搜索/系列三类来源（插画与小说均可）；在 User/Search/系列模式下用「存为计划任务」一键保存，计划完整沿用当前的下载设置与筛选条件并在后台逐作品过滤，可对单个任务授权使用管理员保存的 Cookie；自动检测 Pixiv「过度访问」站内信警告并暂停同账号任务（避免删号）、登录态失效或单作品连续失败时挂起任务，三种自动暂停均发邮件通知，可在管理页手动暂停/恢复或按账号「无视风险继续 / 稍后继续」
