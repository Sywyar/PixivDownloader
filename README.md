...
- Windows 打包需要 PowerShell；构建 MSI 还需要 WiX Toolset 的 `heat.exe`、`candle.exe`、`light.exe`
...
默认产物会输出到 `build/out/`。如果启用 MSI，脚本会要求本机存在 WiX 工具链；如果启用离线便携版或 `with-ffmpeg` MSI，脚本会自动准备 FFmpeg 文件。
