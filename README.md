...
登录后打开 `http://localhost:6999/pixiv-batch.html`，点击页面顶部的「🧩 油猴脚本」卡片展开，点击对应脚本的「⬇ 安装」按钮。部署在非 localhost 服务器时，`
@connect` 会自动替换为实际地址。
...
Tampermonkey 的 `GM_xmlhttpRequest` 受 `@connect` 白名单限制，脚本默认只允许连接 `localhost`。若后端部署在其他机器，需要手动修改每个脚本的
`@connect` 声明：
...
| `Pixiv 单作品图片下载器(Java后端版).user.js`        | 单作品页下载                                        |
| `Pixiv 单作品图片下载器(Local download).user.js` | 单作品页下载（浏览器本地下载，无需 Java 后端）                    |
| `Pixiv User 批量下载器.user.js`               | 用户主页批量下载                                      |
| `Pixiv URL 批量导入作品下载器.user.js`            | 批量导入作品下载（兼容 One-Tab，N-Tab 等标签页管理插件导出格式）       |
| `Pixiv 页面批量下载器.user.js`                  | 页面 DOM 抓取（支持Pixiv全站抓取）                        |

> **如果只想装一个脚本，优先安装 `Pixiv All-in-One.user.js`。** `Local download` 仍保持独立脚本，不包含在多合一内。
> **如果多合一脚本遇到问题，尝试对应的独立脚本。** ，随后附带请附带复现步骤和环境信息提交 issue。
> **推荐优先使用网页端**：`http://localhost:6999/pixiv-batch.html` 支持批量导入作品、User 模式、Search 模式，无需安装油猴脚本即可完成批量下载。
...
> 在线便携版默认不内置 FFmpeg，但会优先复用系统 PATH、程序目录或用户目录中的 FFmpeg。若你已经安装 FFmpeg，下载在线便携版即可；仅在系统里没有
> FFmpeg 且需要 Ugoira 动图转 WebP 时，再在 GUI 的“状态”页点击“下载 FFmpeg”。普通图片下载不受影响。
...
- 每行一条，格式为 `url | title`
- 示例：`https://www.pixiv.net/artworks/12345678 | 示例标题`
- `title` 可留空，下载前会自动获取真实标题
...
工具相关操作统一改为通过 GUI 启动：启动应用后，打开桌面 GUI 的「工具」标签页即可直接使用以下工具：

| 工具          | 说明                                                                                                                                                          |
|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 图片分类工具      | 打开独立的图片分类窗口，用于整理已下载图片；运行时不会主动停止后端                                                                                                                           |
| 数据库目录有效检查工具 | 检查数据库中的目录字段是否仍然可访问；启动前会自动停止后端，关闭窗口后自动恢复                                                                                                                     |
| 数据库数据回填工具   | 通过 `ArtworksBackFill` 一次性补全 `artworks` 表缺失的 `author_id`、`R18`、`is_ai`、`description` 以及 `tags` / `artwork_tags` 数据；支持数据库路径、代理、请求延迟、处理条数和试运行，并可直接打开 HTML 日志页面 |
...
访问 `http://localhost:6999/pixiv-gallery.html` 查看本地已下载作品画廊；`monitor.html` 和 `pixiv-batch.html` 顶部也提供了 `GALLERY` 入口，方便从下载监控或批量下载页直接跳转。
...
- Windows 打包需要 PowerShell；构建 MSI 前还需要先安装 WiX Toolset，并确保 `heat.exe`、`candle.exe`、`light.exe` 可用。当前 `scripts/package-local.ps1` 直接调用的是这套传统命令行工具，因此请安装带这些可执行文件的 WiX Toolset 版本。
- WiX Toolset 官方说明：https://docs.firegiant.com/wix/
- WiX Toolset v3 发布页（可下载包含上述工具的安装包 / 二进制包）：https://github.com/wixtoolset/wix3/releases
- 如果要构建离线便携版或 `with-ffmpeg` MSI，本地还需要可用的 `curl.exe`，打包脚本会下载或复用 `build/ffmpeg` 中的 FFmpeg 载荷
...
### 4. 构建 Windows 便携版 / EXE / MSI

仓库自带的 [`scripts/package-local.ps1`](./scripts/package-local.ps1) 会统一完成本地打包。它会先生成 `Pixiv All-in-One.user.js`，再执行 Maven `package`，然后用 `jlink` 生成精简运行时、用 `jpackage` 生成带 `PixivDownload.exe` 的 app-image，并按参数继续产出在线便携版、离线便携版和 MSI。

```powershell
$env:JAVA_TOOL_OPTIONS='-Dfile.encoding=UTF-8'

# 仅生成便携版（包含 PixivDownload.exe），不构建 MSI
...
- `-SkipOfflinePortable`：跳过离线便携版
- `-SkipMsi`：跳过 MSI，只保留便携版产物
- `-MsiCultures zh-CN,en-US`：指定 MSI 语言
- `-MsiVariants with-ffmpeg,no-ffmpeg`：指定 MSI 变体
- `-RedownloadFfmpeg`：强制重新下载 FFmpeg 载荷

默认产物会输出到 `build/out/`。如果启用 MSI，脚本会检查本机是否已安装并配置好 WiX Toolset（`heat.exe`、`candle.exe`、`light.exe`）；如果启用离线便携版或 `with-ffmpeg` MSI，脚本会自动准备 FFmpeg 文件。
