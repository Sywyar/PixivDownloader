...
- 统计仪表盘（管理员）：总览卡片、按月下载量折线、下载量 Top 作者、热门标签词云，作者/标签可点击直达画廊筛选
- 疑似重复检测（管理员）：基于感知哈希（dHash）识别实质重复的已下载图片，支持阈值调节、跨作品/全部范围切换与手动扫描回填
- 计划任务（管理员）：后台按固定周期或 Cron 表达式自动发现并下载新作品，支持画师新作/保存的搜索/系列三类来源（插画与小说均可）；在 User/Search/系列模式下用「存为计划任务」一键保存，计划完整沿用当前的下载设置（含最大并发数 / 作品间隔 / 图片间隔 / 实际目录检测）与筛选条件并在后台按设置并发下载、逐作品过滤，可对单个任务授权使用管理员保存的 Cookie；自动检测 Pixiv「过度访问」站内信警告并暂停同账号任务（避免删号）、登录态失效或单作品连续失败时挂起任务，三种自动暂停均发邮件通知，可在管理页手动暂停/恢复或按账号「无视风险继续 / 稍后继续」
- 邮件通知与 SMTP 配置（GUI）：内置常见邮箱服务商预设与「发送测试邮件」，用于接收计划任务过度访问/鉴权失效等需人工介入的通知（邮件正文绝不含 Cookie 或密码）
