...
- 插件管理页（仅管理员）：以卡片列表查看全部插件（内置 / 外置 / 必选但未安装）的状态、来源、版本、运行期阶段、对核心 API 的版本要求与插件间依赖，并对外置插件执行加载 / 启动 / 静默 / 停止 / 卸下 / 重载等运行期生命周期操作（内置插件随主程序编译、只读展示）
- 插件市场页（仅管理员）：从受信仓库浏览、搜索、按分类 / 兼容性筛选、排序并查看插件详情（版本历史 / 依赖 / 大小 / 签名状态），按受控标识发起安装、重启后生效；与插件管理页通过分段控件互链，职责不合并
- 计划任务：后台按固定周期或 Cron 表达式自动发现并下载新作品，支持画师新作/保存的搜索/系列三类来源（插画与小说均可）
- 邮件 / 推送通知：计划任务过度访问、鉴权失效、熔断、作品自动重试达上限等需人工介入的事件，会通过邮件与已配置的推送通道告知，并列出被挂起的任务名称 / ID、任务类型、触发方式、下次预定运行等关键信息；可在通知配置页按类型逐项开关，取消勾选的类型不再通过任何介质发送
- 小说下载与系列合订（TXT/HTML/EPUB，EPUB 支持多级目录和内嵌图片）
- 小说 AI 翻译（需配置大模型）：把正文或整个系列翻译成指定语言并保存到本地，可在原文与译文之间切换查看
- 小说 AI 多角色朗读（beta，可用但尚不稳定；需配置大模型与朗读引擎）：由大模型把整章逐句归属到说话人，每个角色用固定音色逐句合成并连续播放、跟随高亮，首次分析可为旁白选定音色基调（温暖女声 / 沉稳男声 / 中性，可试听）并锁定，可编辑各角色音色并处理冲突，分析结果可缓存重播
- 动图 (Ugoira) 自动转 WebP
- 自定义文件名模板（11 个变量）
