...
- **公式任意外部**：標準インストールの `gui-swing`、`stats`、`posthog`、`duplicate`、`gallery`、`novel`、`notification`、`multi-mode-decision-survey`、`push`、`mail`、`tts`、`ai` と、オンデマンドの `douyin`、`gui-compose`。ない場合は自分の機能だけがなくなり、通常はリカバリーになりません
...
`duplicate`、`gallery`、`novel`、`stats`、`notification`、`tts`、`ai`、`push`、`mail`、デスクトップ GUI provider などの不足・無効化は、それ自体では通常リカバリーを起こしません。各プラグインの機能だけが利用できなくなり、コアに代替実装はありません。

## デスクトップ GUI

アプリケーションが所有する宣言的 UI 文書の「プラグイン」ページは、バックエンドから同じ状態（名前、区分、状態、実行フェーズ、必須、バージョン）を読み取り専用で表示し、Swing と Compose が同じ内容を描画します。「更新」と「Web プラグイン管理を開く」を使えます。インストール、アンインストール、有効化、無効化は Web ページで行い、GUI がプラグインフォルダーを直接走査したり権限チェックを緩めたりすることはありません。

`gui-swing` は標準インストールの既定 provider、`gui-compose` はオンデマンドです。どちらも `process-restart` であり、インストール、更新、有効化 / 無効化、削除、provider 選択の変更にはアプリケーション全体の再起動が必要です。
