...
/**
 * gallery-tools 中统计能力的边界守卫：统计实现（{@link StatsPluginConfiguration} / {@link StatsService} /
 * {@link StatsController} / {@link StatsDto}）
 ...
 *
 * <p>本守卫在 {@code pixivdownload-plugin-gallery-tools} 模块内自包含运行：{@link ClassFileImporter} 只扫描本模块
 ...
 */
class StatsFeatureDependencyGuardTest {
    ...
    void statsModuleIsSelfContained() {
        classes()
                ...
                .because("stats 插件本体只能依赖跨插件契约 plugin.api、核心语义端口 core.stats（core-api）、"
                        ...
                        + "PF4J（外置插件主类 GalleryToolsPf4jPlugin 继承 org.pf4j.Plugin）与 Spring / JDK / Lombok；"
    ...
    @Test
    @DisplayName("gallery-tools 模块应包含合并入口与统计业务类（防守卫 vacuous 通过）")
