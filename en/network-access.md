...
| FFmpeg installer in the application host | `https://github.com/Sywyar/PixivDownloader-Remote-Content/releases/download/ffmpeg-stable/ffmpeg-{windows-x64,linux-x64,linux-arm64,macos-x64,macos-arm64}.zip` and its Release CDN redirects | Downloads the FFmpeg official-stable-source build selected for the current OS and architecture | Triggered only when automatic FFmpeg installation is explicitly selected in the GUI; startup does not download it. Unsupported systems continue to use manual installation; requests use the host proxy settings |
