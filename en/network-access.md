...
| Main-repository SDK publishing workflow and SDK-repository Pages workflow | `https://api.github.com/repos/Sywyar/PixivDownloader-Plugin-SDK` (or the matching `GITHUB_API_URL`), that repository's GitHub Releases and Release-asset CDN, plus GitHub Pages / Actions artifact deployment | The main workflow uses a cross-repository token scoped to the SDK repository to check and create immutable `sdk-api-v*` tags/releases, upload signed assets, and download the public release for verification. The SDK-repository workflow uses its repository `GITHUB_TOKEN` to enumerate and download every non-draft SDK release, validates digests and metadata, and builds the versioned Javadoc Pages artifact. Manual dispatch, tags/releases, Pages settings, and deployment require their corresponding authorization. Installed applications do not access these destinations |
