...
| --- | --- | --- |
| Release / Nightly artifact E2E | GitHub Actions Artifact service, with upload destinations and redirects supplied by the runner | Automatically uploads results, logs, window screenshots, and plugin status from isolated test instances after successful or failed acceptance runs, retaining them for seven days. Uploads use the runtime credentials Actions provides to the current job and exclude publishing private keys. Destinations and proxies follow the runner and official upload-artifact action configuration. Maintainers can change or disable the evidence-upload step in the shared release-acceptance action |
...
| GitHub Actions | GitHub Actions, Artifact, and Release services plus referenced `actions/*` and `softprops/action-gh-release` actions | CI, building, artifact upload, and publishing. The full Quality Gate runs Java tests / SDK contracts and ProGuard artifact verification in parallel jobs, each checking out the same source and using the Maven dependency cache. Cold caches can cause duplicate dependency downloads. Both jobs only have source-read permissions, use no publishing credentials, and transfer no build outputs between jobs. The build-tool settings below still determine download destinations and proxy behavior |
