...
| Main-repository `Gate Checks` workflow | Actions run, job, log, PR, commit, and check-runs APIs under `https://api.github.com/repos/Sywyar/PixivDownloader/`, GitHub App installation and token APIs, the current `origin`, and download redirects returned by the log API | Automatically handles subscribed CI events. When PR-only CI is enabled, it also verifies `master` merges and receives results from the existing manual full Quality Gate on the same commit. A read-only `GITHUB_TOKEN` retrieves execution evidence, and Git fetches the corresponding commit objects. After verification, the official `actions/create-github-app-token` action uses the App private key from the `release` Environment to sign a JWT on the runner, then exchanges it with GitHub for a short-lived installation token limited to `checks:write`. That token updates six checks on the tested PR merge commit and is revoked when the job ends; the private key is not sent to GitHub. Requests include repository, PR / run / job IDs, commit SHAs, and check conclusions. They follow the runner's Git, GitHub CLI, and proxy settings; GitHub supplies log download destinations. Disabling the workflow or App blocks the corresponding required checks rather than bypassing the quality gate |
