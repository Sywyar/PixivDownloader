...
- The multi-user-mode retention survey sends the selected or entered answer and an anonymous, survey-scoped identifier derived one-way from the installation identity. The layout survey keeps its existing solo/multi-mode anonymous-identity rules. Neither exposes the raw installation UUID to the browser or PostHog.
