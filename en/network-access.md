...
| `notification` plugin | `https://sywyar.github.io/PixivDownloader-Remote-Content/announcements/index.json`, adjacent `index.json.sig`, and `.../announcements/<message-id>/<locale>.html` | Reads the public announcement index and its detached Ed25519 signature. The exact index bytes are verified against the embedded official trust root before parsing, followed by validity-window, monotonic-sequence, and per-document SHA-256 checks. Controlled HTML for every locale required by the signed index is downloaded only for an unknown stable ID. Requests disable cookies and send only standard connection metadata such as IP address and User-Agent—no account data, works, local paths, or other credentials. Verification or transport failures preserve the last trusted snapshot. The HTML snapshot is stored locally; an administrator's browser reads only a local authenticated endpoint and no longer connects to the external document | Waits for a random 0–30 minutes after each plugin start, then checks about every 6 hours with ±15% jitter. After a complete trusted import, it stores `ETag` and `Last-Modified` and sends conditional requests only while the signed index remains valid; `304 Not Modified` skips signature and document downloads. HTTP 429 honors `Retry-After` up to 24 hours, while transport and 5xx failures back off through 5 minutes, 15 minutes, 1 hour, and 6 hours. Bounded HTML downloads occur only when an unknown ID is first seen or its signed document digest changes. Existing IDs with unchanged metadata and explicitly deleted IDs do not trigger another document request. The official default plugin set includes `notification`, so an enabled, successfully started plugin accesses it automatically | Uses the inherited host outbound route and can use an enabled global proxy. Disabling or uninstalling `notification` stops checks; stopping or reloading the plugin cancels future polling immediately |
...
| `novel` plugin | `https://www.pixiv.net/ajax/novel/**`, `/ajax/novel/series/**`, `/ajax/novel/series_content/**`, `/ajax/user/**/novels`, `/ajax/user/**/novels/bookmarks`, `/ajax/search/novels/**` | Retrieves novel text, series data, author novel lists, bookmarks, and search results. Authenticated requests use the Pixiv cookie. The download workbench reuses the novel response validated during preview through a short-lived one-time ticket bound to the current owner, work, and acquisition credential, so download submission does not request the same novel text again | Triggered by novel preview, series compilation, novel search, author novel views, and scheduled work. A download immediately following preview reuses that response; direct submissions from compatible clients without a ticket still fetch through the backend, while invalid tickets are rejected | Uses the host Pixiv HTTP capability. Disabling `novel` withdraws the novel pages and capabilities |
...
| Root userscripts, not plugins | User-configured PixivDownloader address, `http://localhost:6999` by default | Calls download submission, queue, status, and SSE APIs and sends the selected works and download options. For novel downloads in local solo mode, it also sends the bounded novel response obtained from Pixiv to the local novel plugin in exchange for a short-lived one-time ticket; it does not send the Pixiv cookie | Triggered when Java-backend, batch, or Toolbox userscript features are used | Local by default. If changed to a remote URL, requests go to that server, but novel-response import is disabled and the backend fetches the novel itself |
...
| --- | --- | --- | --- |
| Plugin market in the application host | Stable builds use `https://raw.githubusercontent.com/Sywyar/PixivDownloader-plugins/master/manifest.json`; Nightly builds use `nightly-manifest.json` in the same repository. Packages normally use GitHub Releases and may redirect to `*.githubusercontent.com` | Downloads the official catalog matching the application's release channel and user-selected plugin packages, then verifies signatures, SHA-256, and size | The packaged application version selects the catalog. `plugin-catalog.enabled` and the built-in official repository default to enabled. Opening or refreshing the market fetches the catalog; explicitly installing a plugin downloads its package. Startup itself does not contact a repository. Up to five redirects are followed and each hop is revalidated; disabling the master switch stops this chain |
| Plugin market in the application host | A public HTTPS `repository.json` entered by the administrator | Previews the declared publisher, catalog, revocation/update endpoints, actual network hosts, and complete public-key fingerprints. The request carries no cookies, account data, artwork, local paths, or other application credentials, and does not request `repository.json.sig` | Triggered only when an administrator submits a preview or confirms trust. Confirmation fetches the descriptor again and requires the same SHA-256. The response is limited to 64 KiB and every address receives public-HTTPS and SSRF checks. `DIRECT_STRICT` follows no redirects; `GITHUB_RELEASES` allows one hop inside the fixed GitHub host boundary. Cancelling or withholding confirmation does not save or enable the repository |
| Plugin market in the application host | The trusted descriptor's HTTPS catalog endpoint; optional `revocations.json` / `repository-update.json` and adjacent `.sig`; plugin JAR/ZIP URLs selected by the catalog | `manifest-v1` fetches a signed manifest. `paged-v2` fetches bounded list, detail, and exact-version pages. Before installation the host refreshes required revocations, resolves the exact version again, downloads the package, and verifies size, SHA-256, publisher signature, and the package descriptor. Re-importing a trusted repository may fetch a continuity proof | Triggered by browse/search/paging, detail view, explicit install/update, or re-import. Startup reads only the local last-good revocation snapshot and does not automatically contact a third-party repository. Requests use the fixed safe network profile mapped from the trusted descriptor. Disable/delete the repository or turn off `plugin-catalog.enabled` to stop later requests |
| FFmpeg installer in the application host | `https://github.com/Sywyar/PixivDownloader-Remote-Content/releases/download/ffmpeg-stable/ffmpeg-release.json`, adjacent `ffmpeg-release.json.sig`, the `ffmpeg-{windows-x64,linux-x64,linux-arm64,macos-x64,macos-arm64}.zip` selected for the current system and architecture, and GitHub Release CDN redirects | Verifies the Ed25519 manifest signature against the application's embedded official trust root, then checks the exact asset name, size, and SHA-256 for the official-stable-source FFmpeg build before extraction. GET requests send only the FFmpeg-installer User-Agent and standard connection metadata such as IP address—no Pixiv cookie, account data, or other application credentials | Triggered only when automatic FFmpeg installation is explicitly selected in the GUI; startup does not download it. Unsupported systems continue to use manual installation. Requests use the host proxy settings; not starting automatic installation disables this chain |
| Optional FFmpeg task in Windows Setup | The same signed manifest, detached signature, fixed `ffmpeg-windows-x64.zip`, and GitHub Release CDN redirects described above | During setup, verifies the manifest signature against the official trust root bundled in the installer, then checks the exact Windows asset name, size, and SHA-256 before extracting it into the application tools directory. Requests to the destination carry no application account or Pixiv credential; a system-proxy connection may use the current Windows user's default proxy credentials | Triggered only when the user explicitly selects the FFmpeg task in Setup. Setup prefers an enabled system HTTP/HTTPS proxy and connects directly when none is found; leaving the task unselected skips the chain entirely |
...
| --- | --- | --- |
| Release / Nightly artifact E2E | GitHub Actions Artifact service, with download and upload destinations and redirects supplied by the runner | Tests the Java standard ZIP, full-offline ZIP and Windows installer in parallel on separate runners. Each group downloads its required distribution and uploads results, logs, window screenshots and plugin status under a distinct artifact name after success or failure, retaining them for seven days. A failed group does not cancel the others; publication requires every group to pass. Transfers use the runtime credentials Actions provides to the current job and exclude publishing private keys. Destinations and proxies follow the runner and official artifact action configuration. Maintainers can change or disable the evidence-upload step in the shared release-acceptance action |
...
| GitHub Actions | GitHub Actions, Artifact, and Release services plus referenced `actions/*` and `softprops/action-gh-release` actions | CI, building, artifact upload, and publishing. The full Quality Gate runs Java tests, SDK contracts and ProGuard artifact verification in three parallel jobs, each checking out the same source and using the Maven dependency cache. Cold caches can cause duplicate dependency downloads. These checks only have source-read permissions, use no publishing credentials, and transfer no build outputs between jobs. After the full QG passes for the same source, Release / Nightly builds the application and publishes plugins in parallel. The current run's Artifact service then transfers the core JAR, verification tool and verified plugins for parallel Java-package and installer assembly. Publishing credentials remain confined to the `release` Environment. The build-tool settings below determine download destinations and proxy behavior |
| Main-repository `Gate Checks` workflow | Actions workflow, run, job, log, PR, commit, and check-runs APIs under `https://api.github.com/repos/Sywyar/PixivDownloader/`, GitHub App installation and token APIs, the current `origin`, and download redirects returned by the log API | Automatically handles subscribed CI events. It reads and checks workflow metadata through the run's `workflow_id`, using the workflow name for identity and the run title for PR association. With PR-only CI enabled, it takes the PR number from native associations or the run name and matches the head repository and branch, and writes checks only for current open PRs targeting `master`. PRs targeting development branches and closed PRs do not receive an App token. A read-only `GITHUB_TOKEN` retrieves evidence, and Git fetches the corresponding commit objects. After verification, the official `actions/create-github-app-token` action uses the App private key from the `release` Environment to sign a JWT on the runner, then exchanges it with GitHub for a short-lived installation token limited to `checks:write`. The token attaches required checks (currently six) to the PR head and retains checks on the tested merge commit for verification after merging. Check details retain the tested commit and actual execution link. If GitHub regenerates the test merge commit, the workflow uses the commit API to verify both parents and the file tree. When all three match, it can reuse the execution without writing checks to the regenerated commit. Each publication writes at most two sets. It rechecks the PR, parents, file tree, and effective execution before and after publishing, and attempts to revoke both sets if they change or a write fails. The token is revoked when the job ends; the private key is not sent to GitHub. After a merge, the workflow verifies the actual `master` commit. Recovery evidence from the existing manual full Quality Gate must also cover the same commit in protected `master` history; it can be triggered through a branch or tag pointing to that commit. Neither verification path writes App checks. Requests include repository, PR / workflow / run / job IDs, commit SHAs, and check conclusions. They follow the runner's Git, GitHub CLI, and proxy settings; GitHub supplies log download destinations. Disabling the workflow or App blocks the corresponding required checks |
| Main-repository SDK publishing workflow | The Central Portal API selected by the official `org.sonatype.central` publishing plugin under the current Central protocol, plus `https://repo1.maven.org/maven2/io/github/sywyar/pixivdownloader/` | Runs only when the SDK identity changes on protected `master`, or when a maintainer explicitly starts release recovery. After the Quality Gate passes for the same source SHA, it uses the Central token and PGP material from the `release` Environment to publish all four SDK coordinates, then downloads public POMs, JARs, sources, Javadocs, signatures, and SHA-256 files for an isolated consumer check. The official Maven plugin and Central service determine the final Portal host/path, which a build proxy may replace. Installed applications never run this flow |
| Main-repository SDK publishing / Pages-build workflows and the SDK-repository Pages caller | `https://api.github.com/repos/Sywyar/PixivDownloader-Plugin-SDK/releases`, `https://github.com/Sywyar/PixivDownloader-Plugin-SDK/releases/download/`, that repository's Release-asset CDN, plus GitHub Pages / Actions artifact deployment | The main publishing workflow uses a cross-repository token from the `release` Environment, scoped to the SDK repository, to check and create immutable `sdk-api-v*` tags/releases, upload signed assets, and download the public release for verification. The main repository's reusable Pages-build workflow does not receive that token; it anonymously reads the public Releases API and assets, validates digests and metadata for every non-draft SDK release, and builds the versioned Javadoc Pages artifact. The SDK repository only calls it at a pinned main-repository commit SHA, then its local deploy job uses `pages: write` and Pages OIDC to publish the artifact. Manual dispatch, tags/releases, Pages settings, and deployment require their corresponding authorization. Installed applications do not access these destinations |
| Manual stable-FFmpeg build workflow | `https://ffmpeg.org/download.html`, `https://ffmpeg.org/releases/`, `https://ffmpeg.org/ffmpeg-devel.asc`, `https://chromium.googlesource.com/webm/libwebp`, Linux runner and macOS Homebrew package sources, and GitHub Actions / Artifact / API / Release | Maintainers trigger it manually on the default branch to resolve and verify the latest official stable FFmpeg source and signature, fetch libwebp at a pinned commit, build five platform assets, generate a release manifest containing exact sizes and SHA-256 values, sign it with the official private key from the `release` Environment, and update the Remote Content `ffmpeg-stable` Release with the cross-repository token. Installed applications do not access these source or build-dependency destinations, and the private key is not sent to download targets |
| Local Windows packaging script `scripts/package-local.ps1` | `ffmpeg-release.json`, `ffmpeg-release.json.sig`, and `ffmpeg-windows-x64.zip` under `https://github.com/Sywyar/PixivDownloader-Remote-Content/releases/download/ffmpeg-stable/`, plus GitHub Release CDN redirects | When building the offline Windows portable by default, uses `curl.exe` to fetch the public signed manifest and asset. It verifies the manifest against the repository's official trust root and checks the exact asset name, size, and SHA-256 before putting FFmpeg into the offline package. Requests carry no application account, Pixiv cookie, or publishing credential; cached files are reused only after successful re-verification. This is triggered when local packaging runs without `-SkipOfflinePortable`; `-RedownloadFfmpeg` forces a fresh download. It follows the build machine's curl/proxy configuration. `-SkipOfflinePortable` skips these requests, and installed applications never run this script |
| Quality Gate and SDK pre-publication checks | The Maven download URL configured by the Wrapper, Maven / Gradle dependency repositories and their redirects; a file URL for the staged SDK repository | Every full QG run checks Compose tests, templates, complete SDK Javadocs and isolated consumers, even when the SDK version is unchanged. SDK staging, templates, documentation and consumer checks run sequentially in one job and reuse build outputs; Compose tests follow the ProGuard build. Isolated consumers first download build plugins and third-party dependencies, then rebuild offline with the exact candidate SDK bytes and verify third-party plugin runtime behavior. Network requests contain artifact coordinates and download paths, with no publishing credentials; test signatures use a temporary third-party key generated during the run. The Wrapper, build tools and isolated consumer settings determine download destinations, proxies and mirrors. Caches can reduce requests; QG provides no separate skip switch. SDK publication reuses the same verification entry and checks the public artifacts again |
| Maven / Maven Wrapper | `https://repo.maven.apache.org/maven2`; public SDK publication checks also use `https://repo1.maven.org/maven2` | Downloads Maven 3.9.11, Java dependencies, and build plugins; after SDK publication, verifies public Central resolution and immutable content |
