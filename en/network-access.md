...
- The multi-user-mode retention survey sends the selected or entered answer, survey ID, survey-scoped anonymous identifier, completion status, event time, event name, and public project token. The layout survey sends the survey response, survey ID, survey-scoped anonymous identifier, app version, current layout, survey schema version, event time, event name, and public project token while keeping its existing solo/multi-mode anonymous-identity rules. Neither sends the raw installation identity, account, cookies, artwork, or local paths.
