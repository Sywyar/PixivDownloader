...
`i18n/locales.json` is the language catalog; Chinese (`zh-CN`) is the source language and English (`en-US`) is the global fallback. Common commands:
...
Ship new Chinese copy and its English translation together. When static resources change, regenerate and commit `pixivdownload-app/src/main/resources/static/i18n-static`. See [the repository i18n workflow](https://github.com/Sywyar/PixivDownloader/blob/master/docs/i18n-workflow.md) for baseline acceptance and hook details.
