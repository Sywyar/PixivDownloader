 ...
 * 插件加载框架（PF4J）类型。
 *
 * <p>装配粒度是<b>外置插件包</b>（一个 PF4J pluginId = {@link #sourcePluginId}）：该包的全部配置类共享同一个
 * 插件 classloader，故归入同一个子 context。当前发布格式要求一个包只提供一个同 id 的
 * {@code PluginInstallation}；两种载体由宿主按同一 package generation 原子固化。
 *
