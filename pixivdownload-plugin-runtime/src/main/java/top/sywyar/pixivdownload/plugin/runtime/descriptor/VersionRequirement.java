 ...
 */
public record VersionRequirement(int major, int minor, boolean present, boolean valid, String raw) {

    private static final VersionRequirement UNSPECIFIED =
            new VersionRequirement(-1, -1, false, true, null);
    ...
    public static VersionRequirement of(int major, int minor) {
        return new VersionRequirement(major, minor, true, true, major + "." + minor);
    }
    ...
    public static VersionRequirement parse(String raw) {
        ...
        try {
            ...
            return new VersionRequirement(major, minor, true, true, raw);
     ...
     * 无效声明 → 恒不满足；否则委托 {@link SdkVersion#isCompatible(int, int, int, int)}。
     */
