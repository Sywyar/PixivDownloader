...
test('来源选择先按作品类型收窄并对无上下文的多重匹配拒绝歧义', async () => {
    ...
    assert.throws(() => runtime.captureForMode('user', {}), /ambiguous/);
...
test('同步宿主 API 明确拒绝 thenable，credentialActions 仍可异步', async () => {
    ...
    assert.throws(() => runtime.captureForMode('user', {}), /must return synchronously/);
...
test('matches thenable 被当作违约贡献隔离', async () => {
    ...
    assert.throws(() => runtime.captureForMode('user', {}), /unavailable/);
});
