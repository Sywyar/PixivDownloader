 ...
 *   C) 结果渲染 renderInstallResultHtml：REJECTED_EMPTY 稳定显示；accepted+effectiveAfterRestart 显示「重启后生效」；
...
function installResponse(over) {
    return Object.assign({
        outcome: 'INSTALLED', accepted: true, effectiveAfterRestart: true, status: 200,
...
(function () {
    const installed = PM.buildInstallResult(installResponse(
        { outcome: 'INSTALLED', accepted: true, effectiveAfterRestart: true,
            pluginId: 'ext-demo', version: '1.2.0' }));
    eq('INSTALLED tone=ok', installed.tone, 'ok');
    ok('INSTALLED accepted', installed.accepted === true);
    ok('INSTALLED effectiveAfterRestart', installed.effectiveAfterRestart === true);
...
// —— C) renderInstallResultHtml：稳定显示 / 重启后生效 / 转义 ——
const RESTART_NOTE = '已安装到本地插件目录，将在下次重启后生效。';
...
// C-2) accepted=true + effectiveAfterRestart=true → 渲染「重启后生效」+ ok 色调 + 字段。
(function () {
    const html = PM.renderInstallResultHtml(PM.buildInstallResult(installResponse(
        { outcome: 'INSTALLED', accepted: true, effectiveAfterRestart: true, status: 200,
            message: '插件安装成功', pluginId: 'ext-demo', version: '1.0.0' })));
    ok('accepted 渲染「重启后生效」语义', html.indexOf(RESTART_NOTE) !== -1);
    ...
    // 不暗示已热加载：结果区只说「重启后生效」，不出现「已加载 / 已启动」等运行期措辞。
    ok('accepted 不暗示已加载', html.indexOf('已加载') === -1 && html.indexOf('已启动') === -1);
})();

// C-2b) DUPLICATE：effectiveAfterRestart 仍为 true（包在 plugins/、重启后生效）→ 渲染重启提示 + info 色调。
(function () {
    const html = PM.renderInstallResultHtml(PM.buildInstallResult(installResponse(
        { outcome: 'DUPLICATE', accepted: true, effectiveAfterRestart: true, message: '该版本插件已安装' })));
    ok('DUPLICATE info 色调', html.indexOf('pm-install-result-box--info') !== -1);
    ok('DUPLICATE 渲染重启提示', html.indexOf(RESTART_NOTE) !== -1);
})();
