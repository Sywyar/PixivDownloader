 ...
 *   <li><b>Bean 实例链</b>：{@link ScheduledWorkRunnerRegistry} 保存 probe loader 拥有的执行器 Bean 实例
 *       （实例经其 {@code Class} 反向持有定义它的 classloader）。</li>
 * </ul>
 *
...
class PluginClassLoaderLeakProbeTest {
    ...
    void unregisteringReleasesPluginClassLoader() throws Exception {
        ...
        WeakHandles handles = registerAssertAndTearDown(
                web, routes, statics, i18n, navigation, userscripts, sources, runners, streams);

        // —— 注销后（确定性引用链检查：业务级「无残留引用」硬性保证，与环境无关）——
        assertNoResidualReference(routes, statics, i18n, navigation, userscripts, sources, runners, streams);
        ...
        if (!clCollected) {
            ...
            Assumptions.abort("probe classloader 未在本环境被 GC 回收，但确定性引用链已确认各注册中心无残留——"
                    ...
                    routes, statics, i18n, navigation, userscripts, sources, runners, streams, handles.runner()));
        }
    ...
    private static WeakHandles registerAssertAndTearDown(
            ...
            ScheduledSourceRegistry sources, ScheduledWorkRunnerRegistry runners, PluginStreamRegistry streams)
            throws Exception {
        ...
        sources.register(PLUGIN_ID, plugin.scheduledSources());
        runners.register(List.of(runner));
        ...
        assertThat(sources.resolve(SOURCE_TYPE)).isPresent();
        assertThat(runners.resolve(RUNNER_KIND)).isPresent();
        ...
        sources.unregister(PLUGIN_ID);
        runners.unregister(RUNNER_KIND);
    ...
    private static void assertNoResidualReference(
            ...
            NavigationRegistry navigation, UserscriptRegistry userscripts, ScheduledSourceRegistry sources,
            ScheduledWorkRunnerRegistry runners, PluginStreamRegistry streams) {
        ...
        assertThat(sources.resolve(SOURCE_TYPE)).isEmpty();
        assertThat(runners.resolve(RUNNER_KIND)).isEmpty();
    ...
    private static String leakDiagnostic(
            RouteAccessRegistry routes, StaticResourceRegistry statics, WebI18nBundleRegistry i18n,
            NavigationRegistry navigation, UserscriptRegistry userscripts, ScheduledSourceRegistry sources,
            ScheduledWorkRunnerRegistry runners, PluginStreamRegistry streams, WeakReference<?> weakRunner) {
        return " 诊断["
                ...
                + ", source=" + sources.resolve(SOURCE_TYPE).isPresent()
                + ", runner=" + runners.resolve(RUNNER_KIND).isPresent()
     ...
     * 注册进 {@link ScheduledWorkRunnerRegistry} 后构成「Bean 实例 → 定义它的 classloader」泄漏链的探测点。
