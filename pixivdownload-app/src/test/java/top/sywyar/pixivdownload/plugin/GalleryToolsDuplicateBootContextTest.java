...
@SpringBootTest(properties = {
        ...
        "pixivdownload.plugins-dir=target/test-runtime/plugins-external-gallery-tools-duplicate",
...
class GalleryToolsDuplicateBootContextTest {

    private static final String DUPLICATE_CLASSES_PROPERTY = "gallery-tools.plugin.classes";
    private static final Path PLUGINS_DIR = Path.of("target/test-runtime/plugins-external-gallery-tools-duplicate");
    ...
    void duplicateWebContributionsAreClassloaderAware() {
        ...
        assertThat(routeAccessRegistry.routes())
                .filteredOn(r -> r.pluginId().equals("gallery-tools")
                        && r.route().pathPattern().contains("duplicates"))
        ...
        assertThat(staticResourceRegistry.resources())
                .filteredOn(s -> s.pluginId().equals("gallery-tools")
                        && s.contribution().publicPathPrefix().startsWith("/pixiv-duplicates"))
    ...
    void externalDuplicateChildContextHostsBeansAndControllerMapping() throws Exception {
        ...
        assertThat(queueOperationRegistry.operationsForOwner("gallery-tools"))
    ...
    void duplicateMaintenancePublicationFollowsExternalLifecycle() {
        ...
        ConfigurableApplicationContext initialContext =
                externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
        List<MaintenanceTask> initialTasks = duplicateMaintenanceTasks();
        assertThat(initialTasks).singleElement();

        lifecycleCoordinator.stop("gallery-tools");
        ...
        assertThat(externalPluginContextManager.contextFor("gallery-tools").orElseThrow())
