 ...
 * 三者都不被后续改动悄悄破坏。纯静态资源内容断言（不启 Spring 上下文），与 {@code NavigationMarkupGuardTest}
 * 同一「按 classpath 资源读取各模块 static/」的形态。
 * <p>具体守住：
 * <ul>
 *   <li>下载页 {@code pixiv-batch.html} 加载共享挂载 helper {@code /js/pixiv-vue.js}（否则 {@code window.PixivVue}
 *       缺失、Vue 路径静默回退命令式，等于没接线）；</li>
 ...
 *   <li>共享 helper {@code pixiv-vue.js} 暴露既定 API 面（{@code ensure} / {@code mount}）。</li>
 * </ul>
 */
...
class NovelSearchVueRenderingContractTest {
    ...
    @Test
    @DisplayName("下载页加载共享 Vue 挂载 helper /js/pixiv-vue.js（否则 window.PixivVue 缺失、Vue 路径静默回退）")
    void downloadPageLoadsVueMountHelper() throws IOException {
        assertThat(read(BATCH_HTML))
                .as("pixiv-batch.html 必须加载 /js/pixiv-vue.js，页面才能据其挂载 Vue")
    ...
    @Test
    @DisplayName("共享 helper pixiv-vue.js 暴露既定 API（ensure / mount），供搜索网格与后续槽位复用")
    void vueHelperExposesExpectedApi() throws IOException {
        String js = read(VUE_HELPER);
        assertThat(js).as("应提供按需懒加载运行时的 ensure()").contains("ensure:");
        assertThat(js).as("应提供挂载入口 mount()").contains("mount:");
        assertThat(js).as("应挂到 window.PixivVue 命名空间").contains("PixivVue");
    }
}
