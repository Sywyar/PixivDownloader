...
import top.sywyar.pixivdownload.stats.StatsService;

import static org.assertj.core.api.Assertions.assertThat;

/**
 * 禁用语义（真实 Spring 上下文）：同时禁用 stats / gallery / duplicate 三个功能插件，并尝试禁用核心插件
 * （{@code plugins.core.enabled=false}）。验证：
 * <ul>
 *   <li>三个插件的托管业务 Bean 缺席（含 duplicate 的 UI 查询服务 {@link DuplicateService}——它现在随插件禁用缺席）；</li>
...
class FeaturePluginsDisabledContextTest {
    ...
    @Test
    @DisplayName("stats / gallery / duplicate 退出活动快照，core 仍在场（core 配置被忽略）")
    void disabledFeaturesLeaveSnapshotCoreStays() {
        assertThat(pluginRegistry.plugins()).extracting(PixivFeaturePlugin::id)
                ...
                .doesNotContain("stats", "gallery", "duplicate");
        assertThat(pluginRegistry.disabledPlugins()).extracting(PixivFeaturePlugin::id)
                .containsExactlyInAnyOrder("stats", "gallery", "duplicate");
    ...
    @Test
    @DisplayName("三个功能插件的托管业务 Bean 全部缺席")
    void disabledFeatureBeansAbsent() {
        assertThat(context.getBeanNamesForType(StatsController.class)).isEmpty();
        assertThat(context.getBeanNamesForType(StatsService.class)).isEmpty();
