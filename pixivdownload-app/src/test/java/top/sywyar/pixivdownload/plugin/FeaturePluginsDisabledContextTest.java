...
@DisplayName("gallery/novel 外置缺失且尝试禁用 core 的真实上下文语义")
class FeaturePluginsDisabledContextTest {
    ...
    @Test
    @DisplayName("gallery/novel/duplicate 未安装，core 仍在场（core 配置被忽略）")
    ...
    @Test
    @DisplayName("gallery/novel 外置包缺失时 route/static/i18n/nav/ui 贡献缺席，duplicate 外置 Bean 不在 core-only 上下文")
    void missingExternalFeatureContributionsAbsent() {
        ...
        assertThat(webI18nBundleRegistry.resolve("novel")).isNull();
    ...
    @Test
    @DisplayName("novel 外置包缺失时小说下载 Bean 不在 core-only 上下文")
