...
/**
 * gallery-tools 外置插件启动期加载的端到端集成测试：用<b>真实</b>合并插件 jar（由本模块编译产物 + 根部
 ...
 * 由 {@link GalleryToolsStatsBootContextTest} 单独验证，不在本类重复启动父子 Spring context。
 *
 * <p>构建产物目录经 surefire 系统属性 {@code gallery-tools.plugin.classes} 传入（指向
 * {@code pixivdownload-plugin-gallery-tools/target/classes}，reactor 中先于 app 构建）；未就绪时（如 IDE 未触发 reactor
...
class GalleryToolsExternalPluginIntegrationTest {
    ...
    void loadExternalStatsJar() throws IOException {
        ...
        Assumptions.assumeTrue(statsClasses != null && Files.isDirectory(statsClasses),
                "gallery-tools 插件构建产物未就绪，跳过真实 jar 加载验证");
    ...
    void discoveryBridgesStatsAsExternalSource() {
        ...
        assertThat(discovery.discovered()).extracting(DiscoveredFeaturePlugin::featurePluginId)
                .contains("gallery-tools");

        PluginRegistry registry = new PluginRegistry(List.of(), new PluginToggleProperties(), discovery);
        assertThat(registry.plugins()).extracting(PixivFeaturePlugin::id).contains("gallery-tools");
        assertThat(registry.source("gallery-tools")).contains(PluginSource.EXTERNAL);
    ...
    void externalStatsContributionsRegisterClassloaderAware() {
        ...
        assertThat(requestLeases.currentOwner("gallery-tools")).isPresent();

        // navigation：stats 导航项进入 NavigationRegistry
        assertThat(navigation.navigation())
                .anyMatch(n -> n.pluginId().equals("gallery-tools"));
        ...
        List<StaticResourceRegistry.RegisteredStaticResource> statsResources =
                staticResources.resources().stream()
                        .filter(s -> s.pluginId().equals("gallery-tools")
                                && s.contribution().publicPathPrefix().startsWith("/pixiv-stats"))
                        .toList();
    ...
    private static Path locateStatsClasses() {
        String configured = System.getProperty(GALLERY_TOOLS_CLASSES_PROPERTY);
