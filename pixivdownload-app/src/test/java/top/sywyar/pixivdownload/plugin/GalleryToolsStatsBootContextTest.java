 ...
 * {@code pixivdownload-plugin-gallery-tools} 的编译产物 + 根部 {@code plugin.properties} 组装）放进固定的工作目录
 * {@code target/test-runtime/plugins-external-gallery-tools-stats}，经系统属性 {@code pixivdownload.plugins-dir} 指向它，
 ...
 * </ul>
 *
 * <p>这条用例覆盖的是组件级 {@link GalleryToolsExternalPluginIntegrationTest} <b>没有</b>覆盖的那一段——后者手工
 ...
 *
 * <p>合并插件构建产物目录经 surefire 系统属性 {@code gallery-tools.plugin.classes} 传入；未就绪时
...
class GalleryToolsStatsBootContextTest {

    private static final String STATS_CLASSES_PROPERTY = "gallery-tools.plugin.classes";
    private static final Path PLUGINS_DIR = Path.of("target/test-runtime/plugins-external-gallery-tools-stats");
    ...
    void runtimeStatusAndDiscoveryBeansSeeStats() {
        ...
        assertThat(pluginRuntimeStatus.startedPluginIds()).contains("gallery-tools");
    ...
    void pluginRegistryBeanContainsStatsAsExternal() {
        assertThat(pluginRegistry.plugins()).extracting(PixivFeaturePlugin::id)
                .containsExactlyInAnyOrder(
                        "core", "plugin-market", "gallery-tools");
        assertThat(pluginRegistry.source("gallery-tools")).contains(PluginSource.EXTERNAL);
        ...
        assertThat(pluginRegistry.registeredPlugins())
                .filteredOn(rp -> !rp.id().equals("gallery-tools"))
    ...
    void routeRegistryBeanExposesStatsRoutes() {
        assertThat(routeAccessRegistry.routes())
                .filteredOn(r -> r.pluginId().equals("gallery-tools")
                        && r.route().pathPattern().contains("stats"))
    ...
    void navigationRegistryBeanExposesStats() {
        assertThat(navigationRegistry.navigation()).anyMatch(n -> n.pluginId().equals("gallery-tools"));
    }
    ...
    void staticAndI18nRegistryBeansAreClassloaderAware() {
        ...
        StaticResourceRegistry.RegisteredStaticResource staticResource =
                staticResourceRegistry.resources().stream()
                        .filter(s -> s.pluginId().equals("gallery-tools")
        ...
        assertThat(staticResourceRegistry.resources().stream()
                .anyMatch(s -> s.pluginId().equals("gallery-tools")
    ...
    void externalStatsChildContextHostsStatsBeans() throws Exception {
        ConfigurableApplicationContext child = externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
    ...
    void externalStatsControllerIsDynamicallyRegistered() throws Exception {
        ...
        assertThat(pluginControllerRegistrar.registeredPluginIds()).contains("gallery-tools");
        assertThat(statsDashboardHandlerBean()).isSameAs(statsControllerBean);

        // 注销后：同 URL 不再命中 controller
        pluginControllerRegistrar.unregisterControllers("gallery-tools");
        assertThat(pluginControllerRegistrar.registeredPluginIds()).doesNotContain("gallery-tools");
        assertThat(statsDashboardHandlerBean()).isNull();

        // 可逆：重新注册恢复（也还原本类其它用例 / AfterAll 依赖的已注册状态）
        int restored = pluginControllerRegistrar.registerControllers("gallery-tools", child);
    ...
    void externalStatsWebContributionsAreRevocable() {
        PluginRegistry.RegisteredPlugin stats = pluginRegistry.registeredPlugins().stream()
                .filter(rp -> rp.id().equals("gallery-tools")).findFirst().orElseThrow();
        ...
        pluginLifecycleService.stop("gallery-tools");
        ...
        assertThat(routeAccessRegistry.routes()).noneMatch(r -> r.pluginId().equals("gallery-tools"));
        assertThat(staticResourceRegistry.resources()).noneMatch(s -> s.pluginId().equals("gallery-tools"));
        assertInstalledGalleryToolsI18nFallback();
        assertThat(navigationRegistry.navigation()).noneMatch(n -> n.pluginId().equals("gallery-tools"));

        // 可逆：重新接入恢复（也还原本类其它用例 / AfterAll 依赖的已注册状态）。
        pluginLifecycleService.start("gallery-tools");
        assertThat(routeAccessRegistry.isDeclared("/api/stats/dashboard", HttpMethod.GET)).isTrue();
        assertThat(staticResourceRegistry.resources()).anyMatch(s -> s.pluginId().equals("gallery-tools"));
        assertActiveStatsI18nBundle();
        assertThat(navigationRegistry.navigation()).anyMatch(n -> n.pluginId().equals("gallery-tools"));
    }
    ...
    void externalStatsLifecycleQuiesceStopStartIsReversible() throws Exception {
        ...
        assertThat(pluginLifecycleService.phase("gallery-tools")).contains(PluginRuntimePhase.STARTED);
        assertThat(routeAccessRegistry.isDeclared("/api/stats/dashboard", HttpMethod.GET)).isTrue();
        assertThat(pluginControllerRegistrar.registeredPluginIds()).contains("gallery-tools");
        ...
        pluginLifecycleService.quiesce("gallery-tools");
        assertThat(pluginLifecycleService.phase("gallery-tools")).contains(PluginRuntimePhase.QUIESCED);
        assertThat(routeAccessRegistry.isDeclared("/api/stats/dashboard", HttpMethod.GET)).isTrue();

        // stop：按序拆除服务足迹——controller 注销、web 贡献注销（URL「未声明」即 AuthFilter 404）、子 context 关闭。
        ConfigurableApplicationContext beforeStop = externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
        pluginLifecycleService.stop("gallery-tools");
        assertThat(pluginLifecycleService.phase("gallery-tools")).contains(PluginRuntimePhase.STOPPED);
        assertThat(pluginControllerRegistrar.registeredPluginIds()).doesNotContain("gallery-tools");
        assertThat(statsDashboardHandlerBean()).isNull();
        assertThat(routeAccessRegistry.isDeclared("/api/stats/dashboard", HttpMethod.GET)).isFalse();
        assertThat(staticResourceRegistry.resources()).noneMatch(s -> s.pluginId().equals("gallery-tools"));
        assertInstalledGalleryToolsI18nFallback();
        assertThat(externalPluginContextManager.contextFor("gallery-tools")).isEmpty();
        ...
        assertThat(staticResourceRegistry.resources()).anyMatch(s -> s.pluginId().equals("gallery-tools"));
        assertActiveStatsI18nBundle();
        ConfigurableApplicationContext afterStart = externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
    ...
    void externalStatsReloadCycleLeavesNoHandlerFromPluginClassLoader() {
        ...
        long initialGeneration = pluginLifecycleService.generation("gallery-tools").orElseThrow();

        // restart：只重建服务足迹，保留代码 generation 与 classloader。
        lifecycleCoordinator.restart("gallery-tools");
        assertThat(pluginLifecycleService.phase("gallery-tools")).contains(PluginRuntimePhase.STARTED);
        assertThat(pluginLifecycleService.generation("gallery-tools")).contains(initialGeneration);
        assertThat(externalStatsClassLoader()).isSameAs(statsCl);

        ConfigurableApplicationContext beforeReload = externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
        lifecycleCoordinator.reload("gallery-tools");
        assertThat(statsDashboardHandlerBean()).isNotNull();
        ConfigurableApplicationContext afterReload = externalPluginContextManager.contextFor("gallery-tools").orElseThrow();
        assertThat(afterReload).isNotSameAs(beforeReload);
        assertThat(beforeReload.isActive()).isFalse();
        assertThat(pluginLifecycleService.generation("gallery-tools").orElseThrow()).isGreaterThan(initialGeneration);
        ...
        assertThat(anyHandlerLoadedBy(statsCl))
                ...
                .isFalse();
        assertThat(externalPluginContextManager.contextFor("gallery-tools")).isEmpty(); // lifecycle 不再持有子 context
        ...
        lifecycleCoordinator.start("gallery-tools");
        assertThat(pluginLifecycleService.phase("gallery-tools")).contains(PluginRuntimePhase.STARTED);
        assertThat(statsDashboardHandlerBean()).isNotNull();
    ...
    private ClassLoader externalStatsClassLoader() {
        return pluginRegistry.registeredPlugins().stream()
                .filter(rp -> rp.id().equals("gallery-tools")).findFirst().orElseThrow().classLoader();
    }
    ...
    private void assertInstalledGalleryToolsI18nFallback() {
        WebI18nBundleRegistry.RegisteredBundle bundle = webI18nBundleRegistry.resolve("gallery-tools");
        assertThat(bundle).isNotNull();
        assertThat(bundle.pluginId()).isEqualTo("gallery-tools");
        assertThat(bundle.load(Locale.SIMPLIFIED_CHINESE)).containsEntry("plugin.name", "画廊工具");
    }
