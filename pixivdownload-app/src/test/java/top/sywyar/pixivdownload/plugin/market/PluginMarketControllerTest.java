...
class PluginMarketControllerTest {
    ...
    void installSuccess() throws Exception {
        ...
        when(marketService.install("official", "demo", "1.0.0", false)).thenReturn(report);
    ...
    void installRecoveryBlockedReturns503() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", false)).thenReturn(new PluginInstallReport(
    ...
    void installUnknownRepository() throws Exception {
        when(marketService.install("ghost", "demo", "1.0.0", false)).thenThrow(new PluginCatalogException(
    ...
    void installDowngradeRejected() throws Exception {
        when(marketService.install("official", "demo", "0.9.0", false)).thenReturn(new PluginInstallReport(
    ...
    void installUnknownVersion() throws Exception {
        when(marketService.install("official", "demo", "9.9.9", false)).thenThrow(new PluginCatalogException(
    ...
    void installTooLarge() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", false)).thenThrow(new PluginCatalogException(
    ...
    void installDownloadFailed() throws Exception {
        ...
        when(marketService.install("official", "demo", "1.0.0", false)).thenThrow(exception);
    ...
    void ignoresArbitraryUrlInBody() throws Exception {
        ...
        verify(marketService).install("official", "demo", "1.0.0", false);
    }
    ...
    void installPassesExplicitIdentityMigrationConfirmation() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", true)).thenReturn(new PluginInstallReport(
                PluginInstallOutcome.INSTALLED, true, true, "demo", "1.0.0", null,
                List.of(), List.of(), List.of()));
        ...
        verify(marketService).install("official", "demo", "1.0.0", true);
    }
