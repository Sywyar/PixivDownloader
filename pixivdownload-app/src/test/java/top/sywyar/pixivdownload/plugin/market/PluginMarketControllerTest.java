...
class PluginMarketControllerTest {
    ...
    void installSuccess() throws Exception {
        ...
        when(marketService.install("official", "demo", "1.0.0", false, false)).thenReturn(report);
    ...
    void installRecoveryBlockedReturns503() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", false, false)).thenReturn(new PluginInstallReport(
    ...
    void installUnknownRepository() throws Exception {
        when(marketService.install("ghost", "demo", "1.0.0", false, false)).thenThrow(new PluginCatalogException(
    ...
    void installDowngradeRejected() throws Exception {
        when(marketService.install("official", "demo", "0.9.0", false, false)).thenReturn(new PluginInstallReport(
    ...
    void installUnknownVersion() throws Exception {
        when(marketService.install("official", "demo", "9.9.9", false, false)).thenThrow(new PluginCatalogException(
    ...
    void installTooLarge() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", false, false)).thenThrow(new PluginCatalogException(
    ...
    void installDownloadFailed() throws Exception {
        ...
        when(marketService.install("official", "demo", "1.0.0", false, false)).thenThrow(exception);
    ...
    void ignoresArbitraryUrlInBody() throws Exception {
        ...
        verify(marketService).install("official", "demo", "1.0.0", false, false);
    }

    @Test
    @DisplayName("POST install 显式身份迁移确认：409 提示后可对同一受控制品确认重试")
    void installPassesExplicitIdentityMigrationConfirmation() throws Exception {
        when(marketService.install("official", "demo", "1.0.0", false, true)).thenReturn(new PluginInstallReport(
                PluginInstallOutcome.INSTALLED, true, true, "demo", "1.0.0", null,
                List.of(), List.of(), List.of()));

        mockMvc.perform(post("/api/plugin-market/official/demo/1.0.0/install")
                        .param("confirmIdentityMigration", "true"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome").value("INSTALLED"));

        verify(marketService).install("official", "demo", "1.0.0", false, true);
    }

    @Test
    @DisplayName("POST install 显式首次信任确认只作用于同一受控制品")
    void installPassesExplicitFirstTrustConfirmation() throws Exception {
        when(marketService.install("custom", "demo", "1.0.0", true, false)).thenReturn(new PluginInstallReport(
                PluginInstallOutcome.INSTALLED, true, true, "demo", "1.0.0", null,
                List.of(), List.of(), List.of()));

        mockMvc.perform(post("/api/plugin-market/custom/demo/1.0.0/install")
                        .param("confirmFirstTrust", "true"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome").value("INSTALLED"));

        verify(marketService).install("custom", "demo", "1.0.0", true, false);
    }
