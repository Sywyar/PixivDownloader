 ...
 *   <li>{@link PluginRegistry} Bean 同时含四个内置插件与外置 {@code stats}，{@code stats} 来源为
...
class StatsExternalPluginBootContextTest {
    ...
    @Test
    @DisplayName("双来源 PluginRegistry Bean：四内置 + 外置 stats，stats 来源 EXTERNAL、classloader 为外置插件 loader")
    void pluginRegistryBeanContainsStatsAsExternal() {
        assertThat(pluginRegistry.plugins()).extracting(PixivFeaturePlugin::id)
                .containsExactlyInAnyOrder(
                        "core", "gallery", "novel", "plugin-market", "stats");
