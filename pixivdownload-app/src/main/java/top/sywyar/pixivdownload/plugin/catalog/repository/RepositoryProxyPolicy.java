 ...
 *   <li>{@link #PROXY_TRUSTED}：仅对用户<b>显式信任</b>的仓库开放、经核心出站代理拉取（配置与界面须给出风险提示）。
 *       该路径的客户端装配属后续插件市场后端流程，当前运行时尚未接线；在此之前对该策略的仓库发起拉取会得到稳定的
 *       「代理策略不支持」错误而非静默降级。</li>
 * </ul>
 *
...
public enum RepositoryProxyPolicy {
    ...
    /** 经核心出站代理拉取，仅对用户显式信任的仓库开放（运行时客户端装配尚未接线）。 */
