...
/**
 * 外置插件 web 贡献（route / static / i18n / navigation / userscript）的统一注册 / 注销入口。
 * 把一个插件向这五类下游注册中心的接入收口到一处，供 {@link ExternalPluginContextManager} 在外置插件子
 ...
 * 五类贡献按与构造期一致的口径重新接入，使「注册 → 注销 → 再注册」后各注册中心快照与首次一致。注册按插件<b>原子</b>：
 * 任一注册中心拒绝（id / 前缀 / namespace / 路由冲突 fail-fast）即把本次已接入的其它注册中心全部回滚后抛出，
 * 不留半接入状态；冲突诊断来自各注册中心、逐字透传。
 *
 * <h2>注销方向（{@link #unregister}）</h2>
 * 按 pluginId 从五类注册中心逐一注销（各注册中心对未注册过的 pluginId 静默返回，故幂等），随后
 ...
 * 的油猴脚本不再残留）。注销后这五类的快照都不含该插件；静态资源的访问回收不靠改 ResourceHandler——路由注销后
...
public class PluginWebContributionRegistrar {
    ...
    /**
     * 按与各注册中心构造期一致的口径接入一个插件的五类 web 贡献（仅接入非空贡献），随后刷新 {@link ScriptRegistry}。
     * 接入按插件原子：任一注册中心抛出（重复 id / 前缀 / namespace / 路由冲突）即回滚本次已接入的其它注册中心并抛出。
     *
    ...
    /**
     * 按 pluginId 注销一个插件的五类 web 贡献（幂等），清其 classloader 的 {@link ResourceBundle} 缓存，
