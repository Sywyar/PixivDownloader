...
final class DesktopPluginStatusController {
    ...
    DesktopPluginStatusController(AppDesktopUiModel owner, DesktopUiHost host) {
        this.owner = owner;
        this.host = host;
    }
    ...
    DesktopUiNode classicPage(Map<String, Runnable> nextActions) {
        ...
        if (recoveryMode) {
            header.add(new DesktopUiNode.Surface(
                    "plugins.recovery",
        ...
        if (!notice.isBlank()) center.add(status("plugins.notice", notice));
        if (statuses.isEmpty() && notice.isBlank()) {
            center.add(text("plugins.empty", "gui.plugins.state.empty", TextStyle.CAPTION));
        }
        for (PluginStatusRow plugin : statuses) center.add(pluginCard(plugin));
        DesktopUiNode actions = row(
                "plugins.actions",
                button(
                        "plugins.refresh",
                        "plugins.refresh",
                ...
                button(
                        "plugins.open",
                        "plugins.open",
                        "gui.plugins.action.open-web",
                        true,
                        nextActions,
                        () -> owner.openWeb("/plugin-manage.html")
                )
        );
